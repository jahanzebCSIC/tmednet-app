"""
Generates all T-MEDNet plots for Capo Caccia - Il Nereo (site 205):
  T-Cycle, Anomaly, Stratification, Thresholds 23-27C  (per year)
  Database_T .zip  +  Stat_Report .xlsx  (merged full history)

Data sources:
  Historical DB:  Downloads/Database_T_205_Capo Caccia (Il Nereo)_202401_202412_2025-03-10.rar
                  (Dec 2023 - Dec 2024, depths 10-40m)
  HOBO campaign:  Downloads/Capo Caccia  (Aug 10 - Dec 12 2025, depths 10-40m)
                  Files have (1) suffix duplicates - cleaned by copy-to-tempdir

Output:  Desktop/temperatures t-mednet/Capo Caccia/
           2024/  -->  T-Cycle, anomaly, stratification, thresholds
           2025/  -->  T-Cycle, anomaly, stratification, thresholds (Aug-Sep data only)
"""
...
# -- Config -------------------------------------------------------------------
SITE_NAME   = "Capo Caccia"
SITE_CODE   = 205
HIST_RAR    = r"C:\Users\jahan\Downloads\Database_T_205_Capo Caccia (Il Nereo)_202401_202412_2025-03-10.rar"
HOBO_DIR    = r"C:\Users\jahan\Downloads\Capo Caccia"
LAT, LON    = 40.558, 8.158      # Capo Caccia, NW Sardinia
SST_LAT, SST_LON = 40.558, 8.158
SST_CSV     = r"C:\Users\jahan\Downloads\Capo Caccia\capo_caccia_sst_copernicus.csv"
...
def load_hobo_dir(folder):
    tmp = tempfile.mkdtemp(prefix='capo_hobo_')
    try:
        ...
        print(f"  Archivos HOBO ({len(txts)}): {[os.path.basename(t) for t in txts]}")
        ...
        # Collapse duplicate depth columns (same-depth files across campaigns)
        col_names = [str(int(d)) for d in depths_int]
        df_all.columns = col_names
        unique_depths = sorted(set(col_names), key=int)
        df_clean = pd.DataFrame(index=df_all.index)
        for dep in unique_depths:
            same = df_all.loc[:, df_all.columns == dep]
            df_clean[dep] = same.bfill(axis=1).iloc[:, 0]
...
def read_database_t_rar(rar_path):
    # Extract RAR using 7-zip, then read the resulting txt
    tmp = os.path.join(os.environ['TEMP'], '_capo_hist_extract')
    os.makedirs(tmp, exist_ok=True)
    subprocess.run([SEVENZIP, 'x', rar_path, f'-o{tmp}', '-y'],
                   capture_output=True, check=True)
    txts = [f for f in os.listdir(tmp) if f.endswith('.txt')]
    if not txts:
        raise FileNotFoundError("No .txt found in RAR")
    txt_path = os.path.join(tmp, txts[0])
    df = pd.read_csv(txt_path, sep='\t', dtype={'Date': str, 'Time': str})
...
# -- 1. Load data -------------------------------------------------------------
print("Cargando Database_T historico (RAR)...")
df_hist = read_database_t_rar(HIST_RAR)
...
# -- 2. Merge: concat + dedup (keep last = HOBO priority) ---------------------
...
if sst_series is None:
    ...
    try:
        ...
        ds = copernicusmarine.open_dataset(
            ...
            minimum_longitude=SST_LON - 0.1, maximum_longitude=SST_LON + 0.1,
            minimum_latitude=SST_LAT - 0.1,  maximum_latitude=SST_LAT + 0.1,
            start_datetime="2024-01-01", end_datetime="2026-05-31",
        )
        sst_series = ds["analysed_sst"].sel(
            latitude=SST_LAT, longitude=SST_LON, method="nearest").to_series()
        ...
        if sst_series.empty:
            print("  Pixel en land mask. Intentando coordenadas cercanas (offshore)...")
            SST_LAT2, SST_LON2 = 40.55, 8.12
            ds2 = copernicusmarine.open_dataset(
                dataset_id="SST_MED_SST_L4_NRT_OBSERVATIONS_010_004_c_V2",
                variables=["analysed_sst"],
                minimum_longitude=SST_LON2 - 0.1, maximum_longitude=SST_LON2 + 0.1,
                minimum_latitude=SST_LAT2 - 0.1,  maximum_latitude=SST_LAT2 + 0.1,
                start_datetime="2024-01-01", end_datetime="2026-05-31",
            )
            sst_series = ds2["analysed_sst"].sel(
                latitude=SST_LAT2, longitude=SST_LON2, method="nearest").to_series()
            if sst_series.mean() > 100:
                sst_series = sst_series - 273.15
            sst_series.index = pd.to_datetime(sst_series.index).tz_localize(None)
            sst_series = sst_series.dropna()
...
def plot_annual_T_cycle(df_year, year, out_dir, hist_df=None):
    ...
    out = os.path.join(out_dir,
        f"capo_caccia_{SITE_CODE}_annual_T_cycle"
...
def plot_anomaly(df_year, year, out_dir, hist_df=None):
    ...
    for depth in depths_anom:
        clim_lbl = f"2023-{target_year-1} Climatology ({depth}m)"
    ...
    t0, t1 = df_year.index[0], df_year.index[-1]
    out = os.path.join(out_dir,
        f"capo_caccia_{SITE_CODE}_anomaly"
        f"_{t0.strftime('%Y%m%d')}_{t1.strftime('%Y%m%d')}.png")
    fig.savefig(out, dpi=DPI, bbox_inches='tight')
    plt.close(fig)
...
def plot_stratification(df_year, year, out_dir):
    ...
    t0, t1 = df_year.index[0], df_year.index[-1]
    out = os.path.join(out_dir,
        f"capo_caccia_{SITE_CODE}_stratification"
        f"_{t0.strftime('%Y%m%d')}_{t1.strftime('%Y%m%d')}.png")
    fig.savefig(out, dpi=DPI, bbox_inches='tight')
    plt.close(fig)
...
def plot_thresholds(df_year, year, out_dir, hist_df=None):
    ...
    for thr in range(23, 28):
        ...
        t0, t1 = df_year.index[0], df_year.index[-1]
        out = os.path.join(out_dir,
            f"capo_caccia_{SITE_CODE}_thresholds_{thr}C"
            f"_{t0.strftime('%Y%m%d')}_{t1.strftime('%Y%m%d')}.png")
        fig.savefig(out, dpi=DPI, bbox_inches='tight')
        plt.close(fig)
