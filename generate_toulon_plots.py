"""
Generates all T-MEDNet plots for Toulon Cap Sicie (site 38):
  T-Cycle, Anomaly, Stratification, Thresholds 23-27C  (per year)
  Database_T .zip  +  Stat_Report .xlsx  (merged full history)

Data sources:
  Historical DB:  Downloads/Database_T_38_Cap Sicie_201401-202509_2025-09-16.zip
  HOBO campaigns: Downloads/Toulon  (3 campaigns Nov 2025 - May 2026, depths 5-50 m)
    Campaign 1: 20251112-20260115  depths 5-50m
    Campaign 2: 20260115-20260311  depths 10-50m (no 5m sensor)
    Campaign 3: 20260311-20260521  depths 5-50m  (files have (1)/(3) suffixes)
...
# -- Config -------------------------------------------------------------------
SITE_NAME   = "Cap Sicié"
SITE_SLUG   = "cap_sicie"          # ASCII slug for plot filenames
SITE_CODE   = 38
HIST_ZIP    = r"C:\Users\jahan\Downloads\Database_T_38_Cap Sicié_201401-202509_2025-09-16.zip"
HOBO_DIR    = r"C:\Users\jahan\Downloads\Toulon"
LAT, LON    = 43.044, 5.847
SST_CSV     = r"C:\Users\jahan\Downloads\Toulon\toulon_sst_copernicus.csv"
...
def _load_single_campaign(files_tuple):
    """Load one campaign's files via DataManager and return a clean DataFrame."""
    dm = DataManager(_C(), [])
    lister = _L()
    dm.openfile(files_tuple, _TB(), lister)
    ...
    df_c, depths_int, _ = dm.list_to_df()
    df_c.index = pd.to_datetime(df_c.index)
    df_c.columns = [str(int(d)) for d in depths_int]
    return df_c.sort_index()


def load_hobo_dir(folder):
    # Copy to tempdir with cleaned names (removes (1), (3) etc. suffixes)
    tmp = tempfile.mkdtemp(prefix='toulon_hobo_')
    try:
        ...
        # Group files by campaign (date-range prefix: parts[1]_parts[2])
        ...
        for f in sorted(os.listdir(tmp)):
            ...
            if len(parts) >= 3:
                ...
                campaigns[key].append(os.path.join(tmp, f).replace('\\', '/'))

        if not campaigns:
            return pd.DataFrame()

        # Load each campaign separately (avoids list_to_df MergeError with same-depth columns)
        camp_dfs = []
        for key in sorted(campaigns.keys()):
            files = tuple(sorted(campaigns[key]))
            print(f"  Campana {key}: {len(files)} archivos")
            df_c = _load_single_campaign(files)
            camp_dfs.append(df_c)

        # Merge all campaigns: later campaigns have priority over earlier ones
        df_combined = camp_dfs[0]
        for df_next in camp_dfs[1:]:
            df_combined = df_next.combine_first(df_combined)
...
print("\nCargando HOBO campanas actuales...")
...
# -- 2. Merge: HOBO priority, historical fills NaN (combine_first)
# Needed because the 2024 HOBO campaign 2 (Oct-Dec) lacks the 5m sensor;
# historical has 5m for that period and must not be overwritten with NaN.
all_depth_cols = sorted(
    {c for src in [df_hist, df_hobo] for c in src.columns}, key=int)
_hist_aligned = df_hist.reindex(columns=all_depth_cols)
_hobo_aligned = df_hobo.reindex(columns=all_depth_cols)
df_full = _hobo_aligned.combine_first(_hist_aligned).sort_index()
...
if sst_series is None:
    ...
    try:
        ...
        if sst_series.empty:
            print("  Pixel en land mask. Buscando pixel valido cercano...")
            # Try slightly offshore (NE of Cap Sicie)
            LAT2, LON2 = 43.06, 5.88
            ds2 = copernicusmarine.open_dataset(
                dataset_id="SST_MED_SST_L4_NRT_OBSERVATIONS_010_004_c_V2",
                variables=["analysed_sst"],
                minimum_longitude=LON2 - 0.1, maximum_longitude=LON2 + 0.1,
                minimum_latitude=LAT2 - 0.1,  maximum_latitude=LAT2 + 0.1,
                start_datetime="2025-05-01", end_datetime="2026-05-31",
            )
            sst_series = ds2["analysed_sst"].sel(
                latitude=LAT2, longitude=LON2, method="nearest").to_series()
            if sst_series.mean() > 100:
                sst_series = sst_series - 273.15
            sst_series.index = pd.to_datetime(sst_series.index).tz_localize(None)
            sst_series = sst_series.dropna()
...
def _add_sst(df, depths_in_df):
    if sst_series is None or sst_series.empty:
        # No Copernicus SST: use shallowest in-situ sensor as 0m proxy
        num_d = [d for d in depths_in_df if d != '0' and d.strip().lstrip('-').isdigit()]
        if num_d:
            shallow = min(num_d, key=int)
            df = df.copy()
            df.insert(0, '0', df[shallow])
...
def plot_annual_T_cycle(df_year, year, out_dir, hist_df=None):
    ...
    for col in plot_cols:
        ...
        # Resample to hourly (HOBO is 30-min; ensures size=360 == 15 days throughout)
        col_hourly = col_full.resample('h').mean()
        nona = col_hourly.dropna()
        if nona.empty:
            continue
        # Segment-wise smooth, splitting only at gaps > 7 days
        # (short sensor gaps < 7 days are bridged naturally, keeping smooth curves)
        diffs = nona.index.to_series().diff()
        gap_pos = list(np.where(diffs > pd.Timedelta('168h'))[0]) + [len(nona)]
        result_vals = np.empty(len(nona))
        seg_start = 0
        for end_pos in gap_pos:
            seg = nona.values[seg_start:end_pos]
            if len(seg) > 0:
                result_vals[seg_start:end_pos] = uniform_filter1d(
                    seg, size=min(360, len(seg)))
            seg_start = end_pos
        smoothed = pd.Series(result_vals, index=nona.index).reindex(col_hourly.index)
    ...
    out = os.path.join(out_dir,
        f"{SITE_SLUG}_{SITE_CODE}_annual_T_cycle"
...
def plot_anomaly(df_year, year, out_dir, hist_df=None):
    ...
    for depth in depths_anom:
        clim_lbl = f"2014-{target_year-1} Climatology ({depth}m)"
    ...
    t0, t1 = df_year.index[0], df_year.index[-1]
    out = os.path.join(out_dir,
        f"{SITE_SLUG}_{SITE_CODE}_anomaly"
        f"_{t0.strftime('%Y%m%d')}_{t1.strftime('%Y%m%d')}.png")
    fig.savefig(out, dpi=DPI, bbox_inches='tight')
    plt.close(fig)
...
def plot_stratification(df_year, year, out_dir):
    ...
    t0, t1 = df_year.index[0], df_year.index[-1]
    out = os.path.join(out_dir,
        f"{SITE_SLUG}_{SITE_CODE}_stratification"
        f"_{t0.strftime('%Y%m%d')}_{t1.strftime('%Y%m%d')}.png")
    fig.savefig(out, dpi=DPI, bbox_inches='tight')
    plt.close(fig)
...
def plot_thresholds(df_year, year, out_dir, hist_df=None):
    ...
    for thr in range(23, 28):
        ...
        t0, t1 = df_year.index[0], df_year.index[-1]
        out = os.path.join(out_dir,
            f"{SITE_SLUG}_{SITE_CODE}_thresholds_{thr}C"
            f"_{t0.strftime('%Y%m%d')}_{t1.strftime('%Y%m%d')}.png")
        fig.savefig(out, dpi=DPI, bbox_inches='tight')
        plt.close(fig)
