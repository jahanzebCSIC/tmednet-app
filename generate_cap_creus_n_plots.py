...
Data sources:
  Historical DB:   Downloads/Database_T_07_Cap de Creus-N_200705-202512_2025-12-17.zip
  HOBO campaign:   Downloads/Cap de creus portalo  (Dec 13 2025 -- May 29 2026, depths 5-40 m, no 35m)
...
Output:  Desktop/temperatures t-mednet/Cap de Creus-N/
           ...
           Database_T_7_Cap de Creus-N_<dates>_<today>.zip
           7_Stat_Report_Cap de Creus-N_<dates>_<today>.xlsx
"""
...
SITE_CODE   = 7
HIST_ZIP    = r"C:\Users\jahan\Downloads\Database_T_07_Cap de Creus-N_200705-202512_2025-12-17.zip"
HOBO_DIR    = r"C:\Users\jahan\Downloads\Cap de creus portalo"
LAT, LON    = 42.319, 3.316
# SST pixel shifted to nearest valid sea pixel (Portalo coords land on land mask in L4 SST product)
SST_LAT, SST_LON = 42.296, 3.350
SST_CSV     = r"C:\Users\jahan\Downloads\Cap de creus portalo\cap_creus_n_sst_copernicus.csv"
...
def read_database_t_zip(zip_path):
    tmp = os.path.join(os.environ['TEMP'], '_capcreus_n_hist_extract')
...
def plot_anomaly(df_year, year, out_dir, hist_df=None):
    ...
    for depth in depths_anom:
        clim_lbl = f"2002-{target_year-1} Climatology ({depth}m)"
...
def plot_stratification(df_year, year, out_dir):
    ...
    if not df_window.empty and plot_dcols:
        ...
        # Interpolate depth gaps (e.g. 15m sensor failure) so contourf has no NaN holes
        df_plot = df_plot.astype(float).interpolate(axis=1, method='linear', limit_direction='both')
