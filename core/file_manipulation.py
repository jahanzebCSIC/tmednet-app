...
def load_data(args, consolescreen=False):
    ...
    try:
        ...
        for ifile in args.files[len(args.files) - args.newfiles:]:
            ...
            datos['GMT'] = gmtout
            try:
                datos['S/N'] = int(re.sub(r'\D', '', good[0][good[0].index('S/N:') + 1]))
            except:
                datos['S/N'] = 'XXXXXXXX'
...
def df_to_txt(df, data, SN):
    ...
    """
    print('writing txt')
    output = (_OUTPUT_DIR + os.sep) + str(data['region']) + '_' + data['datainici'].strftime('%Y-%m-%d') + '_' + data[
...
def zoom_data(data, consolescreen):
    ...
    try:
        ...
        if np.all(~control):
            _arr = np.argwhere(np.array(data['time']) == np.array(startdate))
            idx = int(_arr.flat[0]) if _arr.size > 0 else 0
            time_series[0] = data['time'][idx - 72:idx + 1]
        _si = np.argwhere(np.array(time_series[0]) == np.array(startdate))
        start_index = int(_si.flat[0]) if _si.size > 0 else 0

        if indexes.size != 0:
            _first_spike = int(indexes.flat[0])
            if enddate < data['time'][_first_spike - len(temperatures[1])]:
                _ei = np.argwhere(np.array(time_series[1]) == np.array(enddate))
...
def convert_to_netCDF(filename, df, consolescreen):
    ...
    try:
        xarray.Dataset(df.to_xarray()).to_netcdf((_OUTPUT_DIR + os.sep) + filename + '.nc4')
