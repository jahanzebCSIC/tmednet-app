...
try:
    ...
    def labelLine(*a, **kw): pass
from matplotlib import pyplot as plt
import matplotlib.patches as mpatches

from _paths import IMG_DIR as _IMG_DIR_base
_IMG_DIR = _IMG_DIR_base
...
class HistoricData:
    ...
    def __spike_plot_setter(self, data, prop, target_year, depth, percentile_legend, this_year_legend):
        ...
        plt.savefig((_IMG_DIR + _os.sep) + str(target_year) + '_spike_' + self.site_name + ' ' + depth + '.png')
        ax.remove()
    ...
    def __spike_zoom_setter(self, data, prop, target_year, depth, percentile_legend, this_year_legend):
        ...
        plt.savefig(
            (_IMG_DIR + _os.sep) + str(target_year) + '_spike_Summer Months_' + self.site_name + '_' + depth + '.png')
        ax.remove()
    ...
    def __anomaly_plot_setter(self, data, prop, target_year, depth, last_years_legend, this_year_legend, percentile=False):
        ...
        plt.savefig((_IMG_DIR + _os.sep) + str(target_year) + '_anomalies_' + self.site_name + '_' + depth + '.png')
        ax.remove()
    ...
    def __anomaly_zoom_setter(self, data, prop, target_year, depth, last_years_legend, this_year_legend, percentile=False):
        ...
        if percentile:
            ...
            plt.legend(handles=[handles[0], handles[2], handles[3]],
                       labels=[labels[0]] + ['Historic min.', 'Historic max.'])
        plt.savefig(
            (_IMG_DIR + _os.sep) + str(target_year) + '_anomalies_Summer Months_' + self.site_name + '_' + depth + '.png')
        ax.remove()
    ...
    def __multidepth_anomaly_plot_setter(self, data_dict, prop_dict, target_year, depths, last_years_legend,
        ...
        plt.savefig((_IMG_DIR + _os.sep) + str(int(self.site_id)) + '_4_' + str(target_year) + '_A.png')
        ax.remove()
