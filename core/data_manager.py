...
from _paths import DATA_DIR as _DATA_DIR, OUTPUT_DIR as _OUTPUT_DIR
...
class DataManager:
    ...
    def load_data(self):
        ...
        try:
            ...
            for ifile in self.files[len(self.files) - self.newfiles:]:
                ...
                with open(filein, "r", encoding="latin-1") as f:
                    primera_linea = f.readline().strip()
                    if "Título" in primera_linea or "Title" in primera_linea:
                        lineas = f.readlines()
                        with open(filein, "w", encoding="latin-1") as f:
                            f.writelines(lineas)

                df = pd.read_csv(filein, sep='\t', skiprows=1, header=None, encoding='latin-1')
                col = df.columns
                ...
                f.close()
                # Auto-detect date format by scanning rows until an unambiguous date is found
                _date_fmt = "%d/%m/%y %H:%M:%S"  # default: old T-MEDNet DD/MM/YY
                for _scan_idx in range(len(df)):
                    _p = str(df['Date'].iloc[_scan_idx]).split('/')
                    if len(_p) == 3:
                        try:
                            _f0, _f1 = int(_p[0]), int(_p[1])
                            if len(_p[2]) == 4:           # 4-digit year → always European DD/MM/YYYY
                                _date_fmt = "%d/%m/%Y %H:%M:%S"
                                break
                            elif _f1 > 12:                # middle field is a day > 12 → MM/DD/YY (HOBOware)
                                _date_fmt = "%m/%d/%y %H:%M:%S"
                                break
                            elif _f0 > 12:                # first field > 12 → DD/MM/YY (European)
                                _date_fmt = "%d/%m/%y %H:%M:%S"
                                break
                        except (ValueError, IndexError):
                            continue
                df.index = [datetime.strptime(df['Date'].iloc[i] + ' ' + df['Time'].iloc[i], _date_fmt) for i
                            in
                            range(0, len(df))]
                ...
                datos['GMT'] = gmtout
                try:
                    datos['S/N'] = int(re.sub(r'\D', '', meta[meta.index('S/N:') + 1]))
                except:
                    datos['S/N'] = 'XXXXXXXX'
    ...
    def __load_coordinates(self, region):
        ...
        with open(os.path.join(_DATA_DIR, 'metadata.json'), encoding='utf-8') as f:
            data = json.load(f)
    ...
    def report(self, textbox):
        ...
        for text in self.reportlogger:
            ...
            textbox.insert("end", "=========\n")
        with open(os.path.join(_OUTPUT_DIR, 'report.txt'), 'w') as fr:
        ...
        for text in self.reportlogger:
            if n == 0:
                ...
                pdf.text(text)
        pdf.output(os.path.join(_OUTPUT_DIR, 'report.pdf'))
    ...
    def zoom_data(self, data):
        ...
        if np.argwhere(np.array(time_series[1]) == np.array(enddate)).size == 0:
            _end_pos = int(data['df'].index.get_indexer([enddate])[0])
            time_series[1] = data['df'].index[_end_pos - 72:]
            temperatures[1] = data['df']['Temp'][_end_pos - 72:]
        ...
        try:
            ...
            if np.all(~control):
                _arr = np.argwhere(data['df'].index == startdate)
                idx = int(_arr.flat[0]) if _arr.size > 0 else 0
                time_series[0] = data['df'].index[idx - 72:idx + 1]
            _si = np.argwhere(time_series[0] == startdate)
            start_index = int(_si.flat[0]) if _si.size > 0 else 0

            if indexes.size != 0:
                _first_spike = int(indexes.flat[0])
                if enddate < data['df'].index[_first_spike - len(temperatures[1])]:
                    _ei = np.argwhere(time_series[1] == enddate)
                    indexes = np.array(range(int(_ei.flat[0]) if _ei.size > 0 else 0, len(temperatures[1])))
                else:
                    indexes = np.array(range(_first_spike, len(temperatures[1])))
    ...
    def zoom_data_loop(self):
        ...
        for data in self.mdata:
            ...
            for i in indexes:
                data['df']['Temp'].iloc[int(i) - len(np.array(temperatures[1]))] = 999
            _wh = np.argwhere(np.array(data['df'].index) == time_series[0][start_index])
            _end = int(_wh.flat[0]) if _wh.size > 0 else 0
            for i in range(0, _end):
                data['df']['Temp'].iloc[i] = 999
    ...
    def df_to_geojson(self, df, properties, SN):
        ...
        output_filename = os.path.join(_OUTPUT_DIR, 'dataset.geojson')
    ...
    def df_to_txt(self, df, SN):
        ...
        """
        print('writing txt')
        output = os.path.join(_OUTPUT_DIR,
                    str(self.mdata[0]['region']) + '_'
                    + self.mdata[0]['datainici'].strftime('%Y-%m-%d') + '_'
                    + self.mdata[0]['datafin'].strftime('%Y-%m-%d') + '_merged.txt')
    ...
    def convert_to_netCDF(self, filename, df):
        ...
        try:
            xarray.Dataset(df.to_xarray()).to_netcdf(os.path.join(_OUTPUT_DIR, filename + '.nc4'))
