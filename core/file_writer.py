...
def big_merge(filename1, filename2, output):
    ...
    df2 = pd.read_csv(filename2, sep=r"\s+", skiprows=6)
    ...
    if filename1 == '':
        ...
        df2.to_csv((_OUTPUT_DIR + _os.sep) + output + '.txt', sep='\t', index=False)
    else:
        ...
        dfconc.to_csv((_OUTPUT_DIR + _os.sep) + output + '.txt', sep='\t', index=False)
        return duplicity